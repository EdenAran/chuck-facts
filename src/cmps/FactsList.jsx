...
export const FactsList = ({ facts, sort }) => {
    ...
    return (
        <div className="facts-list">
            <div className="fact">
                ...
                <span className="id header">ID <img src={require('../assets/img/filter.svg').default} alt="filter" /></span>
                <span className="category header">Category <img src={require('../assets/img/filter.svg').default} alt="filter" /></span>
                <span className="date header">Date Created <img className={diff > 0 ? '' : 'up'} src={require('../assets/img/arrow.svg').default} onClick={sortFacts} alt="sort" /><img src={require('../assets/img/filter.svg').default} alt="filter" /></span>
            </div>
            {facts.map(fact =>
                <div className={'fact ' + (isSelected(fact.id) ? 'selected' : '')} key={fact.id} onClick={() => selectFact(fact)}>
                    ...
                    {isSelected(fact.id) && <FactPreview fact={fact.value} />}
                </div>
            )}
        </div >
    )
